...
export const create = async (req, res) => {
  ...
  try {
    ...
    console.log(err);
    return res.sendStatus(400).send("Course create failed. Please try again.");
  }
};
...
export const stripeSuccess = async (req, res) => {
  try {
    // find course
    const course = await Course.findById(req.params.courseId).exec();
    // get user stripe session id from db
    const user = await User.findById(req.user._id).exec();
    // if no stripe session, return
    if (!user.stripeSession.id) return res.sendStatus(400);
    // retrieve user stripe session
    const session = await stripe.checkout.sessions.retrieve(
      user.stripeSession.id
    );
    console.log("STRIPE SESSION SUCCESS => ", session);
    // upon successful payment, push course to user's id
